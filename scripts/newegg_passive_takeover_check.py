#!/usr/bin/env python3
"""Passive CT-log and DNS takeover triage for Newegg HackerOne scope."""
...
OUT_DIR = Path("bug_bounty_cases/08-newegg-bug-bounty/research")
    ...
    for row in rows[:5000]:
        ...
        for raw_name in value.splitlines():
            ...
            if not name or not name.endswith(domain):
                continue
            if any(name == excluded or name.endswith("." + excluded) for excluded in OUT_OF_SCOPE):
                continue
...
def main() -> int:
    max_checks = int(sys.argv[1]) if len(sys.argv) > 1 else 250
    ...
    priority_terms = ("dev", "stage", "staging", "test", "preview", "qa", "sandbox", "old")
    candidates = sorted(
        all_names,
        key=lambda value: (
    ...
    report = {
        "program": "Newegg HackerOne",
        "method": "passive crt.sh plus DNS-over-HTTPS CNAME/A checks",
        "scope_file": "bug_bounty_cases/08-newegg-bug-bounty/scope_notes.md",
        "domains": DOMAINS,
