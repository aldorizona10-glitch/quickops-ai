...
METADATA = FACTORY / "shorts_metadata.csv"
LOG_DIR = ROOT / "logs"
...
def build_youtube_metadata(row: dict[str, str], privacy: str) -> dict:
    description = "\n\n".join(
        [
            row["description"],
            row["hashtags"],
            "QuickOps AI Lab: practical AI workflow and security lessons.",
...
def main() -> int:
    if len(sys.argv) < 2:
        print("Usage: python3 scripts/upload_youtube_short.py <short_id|all>", file=sys.stderr)
        return 2
    ...
    for short_id in selected:
        if short_id not in metadata:
            raise RuntimeError(f"Unknown short id: {short_id}")
        if short_id in uploaded:
            print(f"SKIP: {short_id} already uploaded")
            continue
        path = RENDERED / f"{short_id}-quickops-ai.mp4"
        if not path.exists():
        ...
        append_log(short_id, path, metadata[short_id]["title"], video_id, privacy)
