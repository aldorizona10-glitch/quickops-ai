# Full Automation Revenue Scout Report

Generated: 2026-06-15 11:14:21 WIB

This report lists earning paths where discovery and triage can be automated. It does not claim money is earned until a platform/client/maintainer accepts and pays.

## Automation Reality

- Fully automatable: public search, scoring, duplicate filtering, draft generation, status tracking, reply checks.
- Not fully automatable without risk: KYC, CAPTCHA, account signup, platform screening, interviews, live service calls, payout setup, maintainer acceptance.

## Top Opportunities

### Ask HN: Feedback on an AI-driven "Life RPG" for real-world skill building?

- Source: hacker_news
- Lane: HN Show HN product feedback
- Score: 93
- URL: https://news.ycombinator.com/item?id=48379087
- Context: https://news.ycombinator.com/item?id=48379087
- Automation: auto_discover_auto_draft_manual_send
- Cash path: USD 50-100 paid feedback/testing/workflow offer if public contact exists
- Blocker: Send only if contact is public and relevant; no spam; no account/KYC bypass
- Contact email: 
- Contact URL: 
- Draft subject: Show HN - practical workflow/testing pass
- Next action: If contact exists, review draft and send one specific offer. If only contact URL exists, use form manually.

### Show HN: VisiSign – $0.10 per envelope e-signatures with no monthly fee

- Source: hacker_news
- Lane: HN Show HN technical tester
- Score: 90
- URL: https://visisign.app/
- Context: https://news.ycombinator.com/item?id=48154654
- Automation: auto_discover_auto_draft_manual_send
- Cash path: USD 50-100 paid feedback/testing/workflow offer if public contact exists
- Blocker: Send only if contact is public and relevant; no spam; no account/KYC bypass
- Contact email: 
- Contact URL: https://visisign.app/contact
- Draft subject: Show HN - e-signature workflow testing pass
- Next action: If contact exists, review draft and send one specific offer. If only contact URL exists, use form manually.

### Show HN: Papermill Press – An AI-friendly markup language for PDF generation

- Source: hacker_news
- Lane: HN Show HN product feedback
- Score: 85
- URL: https://news.ycombinator.com/item?id=48477708
- Context: https://news.ycombinator.com/item?id=48477708
- Automation: auto_discover_auto_draft_manual_send
- Cash path: USD 50-100 paid feedback/testing/workflow offer if public contact exists
- Blocker: Send only if contact is public and relevant; no spam; no account/KYC bypass
- Contact email: 
- Contact URL: 
- Draft subject: Show HN - PDF workflow testing pass
- Next action: If contact exists, review draft and send one specific offer. If only contact URL exists, use form manually.

### Show HN: I am running 3 coding agents non-stop over the last 3 days. Here is how

- Source: hacker_news
- Lane: HN Show HN product feedback
- Score: 83
- URL: https://news.ycombinator.com/item?id=48520757
- Context: https://news.ycombinator.com/item?id=48520757
- Automation: auto_discover_auto_draft_manual_send
- Cash path: USD 50-100 paid feedback/testing/workflow offer if public contact exists
- Blocker: Send only if contact is public and relevant; no spam; no account/KYC bypass
- Contact email: 
- Contact URL: 
- Draft subject: Show HN - AI workflow testing pass
- Next action: If contact exists, review draft and send one specific offer. If only contact URL exists, use form manually.

### Show HN: Inspection Credit – read your inspection, draft the negotiation

- Source: hacker_news
- Lane: HN Show HN technical tester
- Score: 75
- URL: https://www.inspectioncredit.com
- Context: https://news.ycombinator.com/item?id=48098179
- Automation: auto_discover_auto_draft_manual_send
- Cash path: USD 50-100 paid feedback/testing/workflow offer if public contact exists
- Blocker: Send only if contact is public and relevant; no spam; no account/KYC bypass
- Contact email: hello@inspectioncredit.com
- Contact URL: https://www.inspectioncredit.com/contact
- Draft subject: Show HN - practical workflow/testing pass
- Next action: If contact exists, review draft and send one specific offer. If only contact URL exists, use form manually.

### Launch HN: Intuned (YC S22) – Build and run reliable browser automations as code

- Source: hacker_news
- Lane: HN Show HN product feedback
- Score: 68
- URL: https://intunedhq.com
- Context: https://news.ycombinator.com/item?id=48445171
- Automation: auto_discover_auto_draft_manual_send
- Cash path: USD 50-100 paid feedback/testing/workflow offer if public contact exists
- Blocker: Send only if contact is public and relevant; no spam; no account/KYC bypass
- Contact email: 
- Contact URL: https://intunedhq.com/contact
- Draft subject: Show HN - AI workflow testing pass
- Next action: If contact exists, review draft and send one specific offer. If only contact URL exists, use form manually.

### Show HN: AG2B – Run the agent loop in the browser, expose your tools via WebMCP

- Source: hacker_news
- Lane: HN Show HN product feedback
- Score: 65
- URL: https://ag2b.ai/docs
- Context: https://news.ycombinator.com/item?id=48308148
- Automation: auto_discover_auto_draft_manual_send
- Cash path: USD 50-100 paid feedback/testing/workflow offer if public contact exists
- Blocker: Send only if contact is public and relevant; no spam; no account/KYC bypass
- Contact email: 
- Contact URL: 
- Draft subject: Show HN - AI workflow testing pass
- Next action: If contact exists, review draft and send one specific offer. If only contact URL exists, use form manually.

### Arrow-air/website: Test Bounty

- Source: github
- Lane: Open-source paid bounty
- Score: 55
- URL: https://github.com/Arrow-air/website/issues/167
- Context: https://github.com/Arrow-air/website/issues/167
- Automation: auto_discover_manual_claim_auto_pr_possible
- Cash path: Bounty payment only after maintainer accepts/merges and pays
- Blocker: Must inspect duplicate attempts and payout rules before coding
- Contact email: 
- Contact URL: 
- Draft subject: 
- Next action: Open issue, check comments/claims, implement only if clean and bounty is explicit.

### HutchyBen/maifetch: [BOUNTY] Rewrite in Rust

- Source: github
- Lane: Open-source paid bounty
- Score: 55
- URL: https://github.com/HutchyBen/maifetch/issues/1
- Context: https://github.com/HutchyBen/maifetch/issues/1
- Automation: auto_discover_manual_claim_auto_pr_possible
- Cash path: Bounty payment only after maintainer accepts/merges and pays
- Blocker: Must inspect duplicate attempts and payout rules before coding
- Contact email: 
- Contact URL: 
- Draft subject: 
- Next action: Open issue, check comments/claims, implement only if clean and bounty is explicit.

### Show HN: Integuru – Integrate with platforms via the source code

- Source: hacker_news
- Lane: HN Show HN product feedback
- Score: 55
- URL: https://www.integuru.com
- Context: https://news.ycombinator.com/item?id=48323417
- Automation: auto_discover_auto_draft_manual_send
- Cash path: USD 50-100 paid feedback/testing/workflow offer if public contact exists
- Blocker: Send only if contact is public and relevant; no spam; no account/KYC bypass
- Contact email: 
- Contact URL: 
- Draft subject: Show HN - practical workflow/testing pass
- Next action: If contact exists, review draft and send one specific offer. If only contact URL exists, use form manually.

### Show HN: DocStreamAI – Automatic invoice/receipt collection for Quickbooks/Xero

- Source: hacker_news
- Lane: HN Show HN workflow buyer
- Score: 50
- URL: https://docstreamai.com/
- Context: https://news.ycombinator.com/item?id=48485900
- Automation: auto_discover_auto_draft_manual_send
- Cash path: USD 50-100 paid feedback/testing/workflow offer if public contact exists
- Blocker: Send only if contact is public and relevant; no spam; no account/KYC bypass
- Contact email: 
- Contact URL: 
- Draft subject: Show HN - practical workflow/testing pass
- Next action: If contact exists, review draft and send one specific offer. If only contact URL exists, use form manually.

### Show HN: An iOS bubble level designed to calibrate away camera bump

- Source: hacker_news
- Lane: HN Show HN workflow buyer
- Score: 50
- URL: https://apps.apple.com/us/app/bubble-level-calibrated/id6767735397
- Context: https://news.ycombinator.com/item?id=48429618
- Automation: auto_discover_auto_draft_manual_send
- Cash path: USD 50-100 paid feedback/testing/workflow offer if public contact exists
- Blocker: Send only if contact is public and relevant; no spam; no account/KYC bypass
- Contact email: 
- Contact URL: 
- Draft subject: Show HN - e-signature workflow testing pass
- Next action: If contact exists, review draft and send one specific offer. If only contact URL exists, use form manually.

### souleater06/mineagent: Bounty

- Source: github
- Lane: Open-source paid bounty
- Score: 45
- URL: https://github.com/souleater06/mineagent/issues/21
- Context: https://github.com/souleater06/mineagent/issues/21
- Automation: auto_discover_manual_claim_auto_pr_possible
- Cash path: Bounty payment only after maintainer accepts/merges and pays
- Blocker: Must inspect duplicate attempts and payout rules before coding
- Contact email: 
- Contact URL: 
- Draft subject: 
- Next action: Open issue, check comments/claims, implement only if clean and bounty is explicit.

### MoonFuji/BountyScout: 🎯 Bounty Alert: 1 fresh fiat bounty

- Source: github
- Lane: Open-source paid bounty
- Score: 45
- URL: https://github.com/MoonFuji/BountyScout/issues/5
- Context: https://github.com/MoonFuji/BountyScout/issues/5
- Automation: auto_discover_manual_claim_auto_pr_possible
- Cash path: Bounty payment only after maintainer accepts/merges and pays
- Blocker: Must inspect duplicate attempts and payout rules before coding
- Contact email: 
- Contact URL: 
- Draft subject: 
- Next action: Open issue, check comments/claims, implement only if clean and bounty is explicit.

### midnightntwrk/contributor-hub: [Bounty]:

- Source: github
- Lane: Open-source paid bounty
- Score: 45
- URL: https://github.com/midnightntwrk/contributor-hub/issues/463
- Context: https://github.com/midnightntwrk/contributor-hub/issues/463
- Automation: auto_discover_manual_claim_auto_pr_possible
- Cash path: Bounty payment only after maintainer accepts/merges and pays
- Blocker: Must inspect duplicate attempts and payout rules before coding
- Contact email: 
- Contact URL: 
- Draft subject: 
- Next action: Open issue, check comments/claims, implement only if clean and bounty is explicit.

### Marvin-The-Bodega-Cat/sleeping-soundly-on-the-rug: Bounty: Chapter 2 \u2014 Bounties Before Bags

- Source: github
- Lane: Open-source paid bounty
- Score: 45
- URL: https://github.com/Marvin-The-Bodega-Cat/sleeping-soundly-on-the-rug/issues/2
- Context: https://github.com/Marvin-The-Bodega-Cat/sleeping-soundly-on-the-rug/issues/2
- Automation: auto_discover_manual_claim_auto_pr_possible
- Cash path: Bounty payment only after maintainer accepts/merges and pays
- Blocker: Must inspect duplicate attempts and payout rules before coding
- Contact email: 
- Contact URL: 
- Draft subject: 
- Next action: Open issue, check comments/claims, implement only if clean and bounty is explicit.

### xevrion-v2/agent-playground: Bounty issue template omits required /bounty marker

- Source: github
- Lane: Open-source paid bounty
- Score: 45
- URL: https://github.com/xevrion-v2/agent-playground/issues/687
- Context: https://github.com/xevrion-v2/agent-playground/issues/687
- Automation: auto_discover_manual_claim_auto_pr_possible
- Cash path: Bounty payment only after maintainer accepts/merges and pays
- Blocker: Must inspect duplicate attempts and payout rules before coding
- Contact email: 
- Contact URL: 
- Draft subject: 
- Next action: Open issue, check comments/claims, implement only if clean and bounty is explicit.

### MoonFuji/BountyScout: 🎯 Bounty Alert: 2 fresh fiat bounties

- Source: github
- Lane: Open-source paid bounty
- Score: 45
- URL: https://github.com/MoonFuji/BountyScout/issues/3
- Context: https://github.com/MoonFuji/BountyScout/issues/3
- Automation: auto_discover_manual_claim_auto_pr_possible
- Cash path: Bounty payment only after maintainer accepts/merges and pays
- Blocker: Must inspect duplicate attempts and payout rules before coding
- Contact email: 
- Contact URL: 
- Draft subject: 
- Next action: Open issue, check comments/claims, implement only if clean and bounty is explicit.

### xevrion-v2/agent-playground: Seeded starter bounty issues omit parent bounty reference

- Source: github
- Lane: Open-source paid bounty
- Score: 45
- URL: https://github.com/xevrion-v2/agent-playground/issues/1035
- Context: https://github.com/xevrion-v2/agent-playground/issues/1035
- Automation: auto_discover_manual_claim_auto_pr_possible
- Cash path: Bounty payment only after maintainer accepts/merges and pays
- Blocker: Must inspect duplicate attempts and payout rules before coding
- Contact email: 
- Contact URL: 
- Draft subject: 
- Next action: Open issue, check comments/claims, implement only if clean and bounty is explicit.

### Freeuni-OOP/final-project-gitbounty: Implement Bounties Page

- Source: github
- Lane: Open-source paid bounty
- Score: 45
- URL: https://github.com/Freeuni-OOP/final-project-gitbounty/issues/68
- Context: https://github.com/Freeuni-OOP/final-project-gitbounty/issues/68
- Automation: auto_discover_manual_claim_auto_pr_possible
- Cash path: Bounty payment only after maintainer accepts/merges and pays
- Blocker: Must inspect duplicate attempts and payout rules before coding
- Contact email: 
- Contact URL: 
- Draft subject: 
- Next action: Open issue, check comments/claims, implement only if clean and bounty is explicit.

### onflow/flow-credit-markets: Bug Bounty

- Source: github
- Lane: Open-source paid bounty
- Score: 45
- URL: https://github.com/onflow/flow-credit-markets/issues/26
- Context: https://github.com/onflow/flow-credit-markets/issues/26
- Automation: auto_discover_manual_claim_auto_pr_possible
- Cash path: Bounty payment only after maintainer accepts/merges and pays
- Blocker: Must inspect duplicate attempts and payout rules before coding
- Contact email: 
- Contact URL: 
- Draft subject: 
- Next action: Open issue, check comments/claims, implement only if clean and bounty is explicit.

### vansh-09/BountyScout: [BOUNTY] Text Needed

- Source: github
- Lane: Open-source paid bounty
- Score: 45
- URL: https://github.com/vansh-09/BountyScout/issues/17
- Context: https://github.com/vansh-09/BountyScout/issues/17
- Automation: auto_discover_manual_claim_auto_pr_possible
- Cash path: Bounty payment only after maintainer accepts/merges and pays
- Blocker: Must inspect duplicate attempts and payout rules before coding
- Contact email: 
- Contact URL: 
- Draft subject: 
- Next action: Open issue, check comments/claims, implement only if clean and bounty is explicit.

### Show HN: SynCodeLive – code and talk with your team along with AI, live

- Source: hacker_news
- Lane: HN Show HN testing buyer
- Score: 45
- URL: https://syncodelive.com/
- Context: https://news.ycombinator.com/item?id=48487791
- Automation: auto_discover_auto_draft_manual_send
- Cash path: USD 50-100 paid feedback/testing/workflow offer if public contact exists
- Blocker: Send only if contact is public and relevant; no spam; no account/KYC bypass
- Contact email: 
- Contact URL: 
- Draft subject: Show HN - practical workflow/testing pass
- Next action: If contact exists, review draft and send one specific offer. If only contact URL exists, use form manually.

### Show HN: Odeva Booking – A unified PMS for holiday parks and campgrounds

- Source: hacker_news
- Lane: HN Show HN product feedback
- Score: 43
- URL: https://news.ycombinator.com/item?id=48376362
- Context: https://news.ycombinator.com/item?id=48376362
- Automation: auto_discover_auto_draft_manual_send
- Cash path: USD 50-100 paid feedback/testing/workflow offer if public contact exists
- Blocker: Send only if contact is public and relevant; no spam; no account/KYC bypass
- Contact email: 
- Contact URL: 
- Draft subject: Show HN - practical workflow/testing pass
- Next action: If contact exists, review draft and send one specific offer. If only contact URL exists, use form manually.

### Show HN: Sandbox AI-app lifecycle, from build to run

- Source: hacker_news
- Lane: HN Show HN testing buyer
- Score: 43
- URL: https://capakit.com/
- Context: https://news.ycombinator.com/item?id=48461287
- Automation: auto_discover_auto_draft_manual_send
- Cash path: USD 50-100 paid feedback/testing/workflow offer if public contact exists
- Blocker: Send only if contact is public and relevant; no spam; no account/KYC bypass
- Contact email: 
- Contact URL: 
- Draft subject: Show HN - practical workflow/testing pass
- Next action: If contact exists, review draft and send one specific offer. If only contact URL exists, use form manually.
